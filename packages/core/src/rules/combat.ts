...
export function resolveCombat(
  ...
  combatLog.push(
    ...
    attackerFirst
      ? `${attacker.character.name} acts first!`
      : `${defenderTarget.name} acts first!`,
    "",
  );
  ...
  while (attackerHp > 0 && defenderHp > 0 && turns < maxTurns) {
    ...
    if ((turns === 1 && attackerFirst) || (turns > 1 && attackerFirst)) {
      const atkResult = calculateDamage(attacker, defenderTarget.defense);
      ...
      const defResult = calculateDamage(
        {
          character: { ...attacker.character, currentHp: defenderHp },
      ...
      const defResult = calculateDamage(
        {
          character: { ...attacker.character, currentHp: attackerHp },
          attack: defenderTarget.attack,
          defense: defenderTarget.defense,
          speed: defenderTarget.speed,
          critRate: defenderTarget.critRate,
